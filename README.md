...
- **秒级刷新**：监测到新命令落盘约 2 秒内重新生成报告，页面 3 秒内自动跟进（数据没变不打扰）
...
打一条命令，几秒内页面就会出现这条记录：后台盯着 `commands.log` 的修改时间，有变化立刻重新生成报告并写出版本标记 `report_meta.json`；浏览器每 3 秒拉一次版本标记，发现更新才整页重载——数据没变时不打扰，你点过的标签、筛选和展开状态也不会被定时重置。
